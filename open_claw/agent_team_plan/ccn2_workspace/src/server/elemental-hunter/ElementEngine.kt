...
object ElementEngine {
    ...
    fun collectElement(
        ...
        val atkBonus = if (element == player.elementAffinity) {
            balance.TILE_REWARD_ATK * player.tileGainMultiplier
        } else {
            balance.TILE_REWARD_MAG * player.tileGainMultiplier
        }
    ...
    private fun applyComboRewards(
        ...
        val rewards = getRewardsForTier(tier)
    ...
    private fun getRewardsForTier(tier: Int): ComboRewards {
        return when (tier) {
            1 -> ComboRewards(atkBonus = BalanceConfig.COMBO_T1_ATK_BONUS, multiplier = 1)
            2 -> ComboRewards(atkBonus = 0, multiplier = 2)
            3 -> ComboRewards(atkBonus = 0, multiplier = 1, atkMultiplier = BalanceConfig.COMBO_T3_ATK_MULTIPLIER)
            else -> ComboRewards()
        }
    }
...
@Serializable
data class ComboRewards(
    val atkBonus: Int = 0,
    val multiplier: Int = 1,
    val atkMultiplier: Double = 1.0
)
